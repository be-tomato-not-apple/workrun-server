...
public interface BookmarkRepository extends JpaRepository<Bookmark, Long> {
    ...
    @Query("SELECT b FROM Bookmark b " +
            "JOIN FETCH b.bookmarkType " +
            "WHERE b.anonymousUser.uuid = :uuid " +
            "AND b.bookmarkType.id = :bookmarkTypeId " +
            "AND (:cursor IS NULL OR b.id < :cursor) " +
            "ORDER BY b.id DESC")
    List<Bookmark> findByUserUuidAndBookmarkTypeIdWithCursor(
            @Param("uuid") String uuid,
            @Param("bookmarkTypeId") Long bookmarkTypeId,
